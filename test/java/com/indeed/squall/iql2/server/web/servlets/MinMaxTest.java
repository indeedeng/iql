...
public class MinMaxTest {
    ...
    public void testAggregateMinMax() throws Exception {
        final List<List<String>> expected = new ArrayList<>();
        expected.add(ImmutableList.of("", "0", "-10" ,"1", "7", "2653"));
        QueryServletTestUtils.testIQL2(OrganicDataset.create(), expected, "from organic yesterday today select min(0,1), min(0,5,7,-10), max(0,1), max(0,5,7,-10), max(oji, ojc)");
    }
}
