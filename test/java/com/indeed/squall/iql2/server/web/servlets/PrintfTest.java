...
public class PrintfTest {
    ...
    public void testGetGroupStats() throws Exception {
        ...
        testAll(OrganicDataset.create(), expected, "from organic yesterday today group by tk select printf(count()/(3*count()), '%.2f')");
    }
    ...
    public void testWithGroupBy() throws Exception {
        // More convoluted than printf(1/3, '%0.2d') to support both IQL1 and IQL2.
        // In IQL1, 1/3 == count() / 3
        testAll(OrganicDataset.create(), ImmutableList.<List<String>>of(ImmutableList.of("", "0.33")), "from organic yesterday today select printf(count()/(3*count()), '%.2f')");
    }
}
