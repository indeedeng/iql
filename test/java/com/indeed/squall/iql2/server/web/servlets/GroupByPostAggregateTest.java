...
public class GroupByPostAggregateTest extends BasicTest {
    ...
    public void groupByFieldHavingDistinct() throws Exception {
        ...
        QueryServletTestUtils.testIQL2(dataset, expected, "from organic yesterday today group by tk HAVING DISTINCT(oji) > 1");
    }
    ...
    public void groupByFieldHavingMultipleConditions() throws Exception {
        final List<List<String>> expected = new ArrayList<>();
        expected.add(ImmutableList.of("d", "141"));
        QueryServletTestUtils.testIQL2(dataset, expected, "from organic yesterday today group by tk HAVING DISTINCT(oji) > 1 AND COUNT() > 100");
    }
    ...
    public void groupByTimeHavingDistinct() throws Exception {
        ...
        QueryServletTestUtils.testIQL2(dataset, expected, "from organic yesterday today group by time(1h) HAVING DISTINCT(oji) > 1");
    }
    ...
    public void groupByHavingFieldMax() throws Exception {
        ...
        expected.add(ImmutableList.of("b", "2"));
        expected.add(ImmutableList.of("c", "4"));
        QueryServletTestUtils.testIQL2(dataset, expected, "from organic yesterday today group by tk[HAVING FIELD_MAX(oji) >= 100]");
    }
    ...
    public void groupByMultipleField() throws Exception {
        ...
        QueryServletTestUtils.testIQL2(dataset, expected, "from organic yesterday today group by tk, oji HAVING PARENT(FIELD_MAX(oji)) > 100");
    }
}
