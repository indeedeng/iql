...
public class IterateHandlers {
    ...
    public static <T> List<T> executeMulti(Session session, String field, Collection<IterateHandler<T>> iterateHandlers) throws ImhotepOutOfMemoryException, IOException {
        ...
        if (session.isIntField(field)) {
            ...
            session.iterateMultiInt(sessionsSubset, sessionMetricIndexes, Collections.<String, Integer>emptyMap(), field, callback, session.timer, false);
            session.timer.pop();
            ...
            session.iterateMultiString(sessionsSubset, sessionMetricIndexes, Collections.<String, Integer>emptyMap(), field, callback, session.timer, false);
            session.timer.pop();
        } else {
