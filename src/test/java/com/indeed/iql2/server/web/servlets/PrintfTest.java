...
public class PrintfTest extends BasicTest {
    ...
    public void testGetGroupStats() throws Exception {
        ...
        QueryServletTestUtils.testAll(expected, "from organic yesterday today group by tk select printf('%.2f', count()/(3*count()))", true);
    }
    ...
    public void testWithGroupBy() throws Exception {
        // More convoluted than printf(1/3, '%0.2d') to support both IQL1 and IQL2.
        // In IQL1, 1/3 == count() / 3
        QueryServletTestUtils.testAll(ImmutableList.of(ImmutableList.of("", "0.33")), "from organic yesterday today select printf('%.2f', count()/(3*count()))");
    }
}
