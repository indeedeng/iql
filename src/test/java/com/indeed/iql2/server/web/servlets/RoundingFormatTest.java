...
public class RoundingFormatTest extends BasicTest {
    ...
    public void testSingleSelectRounding() throws Exception {
        ...
        QueryServletTestUtils.testIQL2(expected, "from jobsearch yesterday today group by country select count() rounding 2");
    }
    ...
    public void testMultipleSelectRounding() throws Exception {
        ...
        QueryServletTestUtils.testIQL2(expected, "from jobsearch yesterday today group by country select count(), count() / 2 rounding 2");
    }
    ...
    public void testSelectPrintfRounding() throws Exception {
        ...
        QueryServletTestUtils.testIQL2(expected, "from jobsearch yesterday today group by country select printf(\"%.2f\", count()), count() rounding 0");
    }
    ...
    public void testSelectRoundingMissingN() throws Exception {
        final List<List<String>> expected = new ArrayList<>();
        try {
            QueryServletTestUtils.testIQL2(expected, "from jobsearch yesterday today group by country select count() rounDing");
    ...
    public void testSelectPrintfNoRounding() throws Exception {
        ...
        expected.add(ImmutableList.of("uk", "2.00", "2"));
        expected.add(ImmutableList.of("us", "3.00", "3"));
        QueryServletTestUtils.testIQL2(expected, "from jobsearch yesterday today group by country select printf(\"%.2f\", count()), count()");
    }
}
