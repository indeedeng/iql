    ...
    public static Query apply(Query query) {
        ...
        for (final GroupByEntry groupBy : query.groupBys) {
            if (groupBy.groupBy instanceof GroupBy.GroupByField && groupBy.filter.isPresent()) {
                ...
                final GroupBy.GroupByField newGroupBy = new GroupBy.GroupByField(groupByField.field, Optional.of(newFilter), groupByField.topK, groupByField.withDefault);
                newGroupBys.add(new GroupByEntry(newGroupBy, Optional.empty(), Optional.empty()));
            } else {
                newGroupBys.add(groupBy);
            }
        }
        return new Query(query.datasets, query.filter, newGroupBys, query.selects, query.formatStrings, query.options, query.rowLimit, query.useLegacy, query.timeZone).copyPosition(query);
    }
}
